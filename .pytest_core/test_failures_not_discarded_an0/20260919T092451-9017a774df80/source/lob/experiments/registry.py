...
def verify_experiment(path: str | Path) -> dict[str, Any]:
    ...
    # Hash the stored document itself. Later optional settings must not mutate
    # the meaning of an already sealed artifact by injecting today's defaults.
    if hashlib.sha256(canonical_json(raw).encode("utf-8")).hexdigest() != metadata["config_sha256"]:
