...
class LOBExecutionEnv(gym.Env):
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        for tr in new:
            fee = self.ledger.apply_trade(tr, self.OWNER, sim.cfg.tick_size)
            self.fills.append(tr)
            self.remaining -= tr.qty
            if self.remaining < 0:
                raise ArithmeticError("parent execution overfilled")
            self.reward_terms["fills"] += sign * (tr.price - self.arrival) / self.arrival * 1e4 \
                * (tr.qty / self.total_qty)
            self.reward_terms["fees"] -= fee / (self.total_qty * self.arrival * sim.cfg.tick_size) * 1e4

        self.risk.available(sim, self.total_qty - self.remaining)
        self.risk.check(sim, self.ledger)
        terminated = self.remaining <= 0
        if self.step_i >= self.n_steps and not terminated:
            # Same objective as report(). Unknown depth invalidates economic
            # metrics; optimisation still receives the explicit completion cost.
        ...
        if terminated:
            self.risk.cancel_all(sim, reason="horizon_cancel")
        if self.record:
    ...
    def report(self, label: str = "RL Agent (PPO)") -> ExecutionReport:
        ...
        return build_report(label, self.side, self.total_qty, self.fills, self.arrival,
                            self.sim.cfg.tick_size, self.n_children, self.sim.t - self.t0, liq,
